...
uint16_t readEEPROM_u16(int address)
{
	return 256 * EEPROM.read(address) + EEPROM.read(address+1);
}
...
static int i;
	...
	writeEEPROM(on, panBatt_B_en_ADDR + offset);
	writeEEPROM(21, panBatt_B_x_ADDR + offset);
	writeEEPROM(3,  panBatt_B_y_ADDR + offset);
	...
	writeEEPROM(on, panBatteryUsed_en_ADDR + offset);
	writeEEPROM(1,  panBatteryUsed_x_ADDR + offset);
	writeEEPROM(4,  panBatteryUsed_y_ADDR + offset); 
	...
	writeEEPROM(off, panWindSpeed_en_ADDR + offset);
	...
	writeEEPROM(100,battv_A_ADDR); //10Volts
	writeEEPROM(100,battv_B_ADDR); //10Volts
	writeEEPROM(6,ch_toggle_ADDR);

	writeEEPROM(1,voltage_calibration_A_ADDR);
	writeEEPROM(1,voltage_calibration_A_ADDR+1);
	writeEEPROM(1,voltage_calibration_B_ADDR);
	writeEEPROM(1,voltage_calibration_B_ADDR+1);
	writeEEPROM(1,current_calibration_mult_A_ADDR);
...
void readSettings() {
	...
	battv_A = (float)EEPROM.read(battv_A_ADDR) / 10.0;
	battv_B = (float)EEPROM.read(battv_B_ADDR) / 10.0;
	...
	voltage_calibration_A = readEEPROM_u16(voltage_calibration_A_ADDR) / 10.0;
	voltage_calibration_B = readEEPROM_u16(voltage_calibration_B_ADDR) / 10.0;
...
void readPanelSettings() {
	...
	setBit(panA_REG[panel], Bu_BIT, readEEPROM(panBatteryUsed_en_ADDR + offset));
	panBatteryUsed_XY[0][panel] = readEEPROM(panBatteryUsed_x_ADDR + offset);
	panBatteryUsed_XY[1][panel] = checkPAL(readEEPROM(panBatteryUsed_y_ADDR + offset));
	...
	setBit(panA_REG[panel], BatB_BIT, readEEPROM(panBatt_B_en_ADDR + offset));
	panBatt_B_XY[0][panel] = readEEPROM(panBatt_B_x_ADDR + offset);
	panBatt_B_XY[1][panel] = checkPAL(readEEPROM(panBatt_B_y_ADDR + offset));
